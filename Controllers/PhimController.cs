...
{    public class PhimController : Controller
    {
        ...
        public async Task<IActionResult> SearchOMDb(string query, string imdbId)
        {
            ...
            try
            {
                OMDbMovieResponse? movieData = null;
        ...
        // GET: Phim/Search
        public async Task<IActionResult> Search(string q = "", int page = 1, string? genre = null, string? country = null, int? year = null, string sort = "relevance")
        {
            ...
            // Load filter options
            model.AvailableGenres = await _context.TheLoaiPhims.OrderBy(g => g.TenTL).ToListAsync();
            model.AvailableCountries = await _context.QuocGias.OrderBy(c => c.TenQG).ToListAsync();
            model.PopularSearches = new List<string> { "Avengers", "Marvel", "DC", "Anime", "Horror", "Comedy", "Romance", "Action" };
            ...
            if (!string.IsNullOrEmpty(q))
            {
                ...
                // Search by title, description, director, actors
                query = query.Where(p => 
                    p.TenPhim.Contains(q) ||
                    (p.MoTaPhim != null && p.MoTaPhim.Contains(q)) ||
                    (p.DaoDien != null && p.DaoDien.Contains(q)) ||
                    (p.DienVien != null && p.DienVien.Contains(q))
                );

                // Apply filters
                if (!string.IsNullOrEmpty(genre))
                {
                    query = query.Where(p => p.MaTL == genre);
                }

                if (!string.IsNullOrEmpty(country))
                {
                    query = query.Where(p => p.MaQG == country);
                }
                ...
                query = sort switch
                {
                    "newest" => query.OrderByDescending(p => p.NgayTao),
                    "oldest" => query.OrderBy(p => p.NgayTao),
                    "rating" => query.OrderByDescending(p => p.ThongKePhim != null ? p.ThongKePhim.DiemTrungBinh : 0),
                    "views" => query.OrderByDescending(p => p.ThongKePhim != null ? p.ThongKePhim.TongLuotXem : 0),
                    "name" => query.OrderBy(p => p.TenPhim),
                    _ => query.OrderByDescending(p => p.TenPhim.Contains(q) ? 1 : 0)
                        .ThenByDescending(p => p.NgayTao)
                };

                // Get total count for pagination
                model.TotalResults = await query.CountAsync();
                model.TotalPages = (int)Math.Ceiling((double)model.TotalResults / pageSize);

                // Get movies for current page
                model.Movies = await query
                    .Skip((page - 1) * pageSize)
                    .Take(pageSize)
                    .ToListAsync();
            }
        ...
        // API: Get related movies
        [HttpGet]
        public async Task<IActionResult> GetRelatedMovies(string genreId, string currentMovieId, int count = 6)
        {
            try
            {
                var relatedMovies = await _context.Phims
                    .Include(p => p.TheLoaiPhim)
                    .Include(p => p.QuocGia)
                    .Where(p => p.MaPhim != currentMovieId && 
                               (p.MaTL == genreId || string.IsNullOrEmpty(genreId)))
                    .OrderByDescending(p => p.NgayTao)
                    .Take(count)
                    .Select(p => new
                    {
                ...
                return Json(relatedMovies);
            }
            catch (Exception)
            {
                return Json(new { error = "Không thể tải phim liên quan" });
            }
        }
    }
}
